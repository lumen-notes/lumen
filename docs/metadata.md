...
| `image`    | Image URL or markdown image syntax | Used as the Open Graph image when sharing the note. Can be a URL (e.g., `https://example.com/image.jpg`) or markdown image syntax (e.g., `![alt text](https://example.com/image.jpg)`). If both `image` and `isbn` are present, `image` takes precedence. |
| `isbn`     | Book ISBN-10 or ISBN-13            | Adds an image of the book cover and an [Open Library](https://openlibrary.org/) link to the top of the note. When sharing the note, the book cover is used as the Open Graph image if no `image` property is set. |
