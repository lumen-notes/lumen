...
export function Switch({ className, ...props }: RadixSwitch.SwitchProps) {
  return (
    <RadixSwitch.Root
      ...
      {...props}
    >
      <RadixSwitch.Thumb className="block h-3 w-3 translate-x-0.5 rounded-full bg-bg transition-transform will-change-transform duration-100 data-[state=checked]:translate-x-[14px] coarse:h-4 coarse:w-4" />
    </RadixSwitch.Root>
  )
}
