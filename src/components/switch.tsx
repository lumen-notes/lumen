...
export function Switch({ className, ...props }: RadixSwitch.SwitchProps) {
  return (
    <RadixSwitch.Root
      ...
      {...props}
    >
      <RadixSwitch.Thumb className="block h-3 w-3 translate-x-0.5 rounded-full bg-bg-card shadow-md transition-transform duration-100 will-change-transform data-[state=checked]:translate-x-[14px] coarse:h-4 coarse:w-4" />
    </RadixSwitch.Root>
  )
}
